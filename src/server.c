...
static void remove_connection_info(int client_fd) {
    ...
    for (int i = 0; i < connection_count; i++) {
        if (connections[i].client_fd == client_fd) {
            free_partial_data(&connections[i]);
...
static void *reaper_thread(void *arg) {
    ...
    while (server->running) {
        ...
        for (int i = connection_count - 1; i >= 0; i--) {
            ...
            if (info->last_activity > 0 && now - info->last_activity > timeout) {
...
static void on_accept(EventLoop *el, int fd, void *userdata) {
    ...
    if (info) {
        server->active_connections++;
        log_message("SERVER", "New connection via event loop");
    }
...
int server_init(Server *server, const Config *config) {
    ...
    if (config->enable_iouring && iouring_available()) {
        log_message("ENGINE", "io_uring available, using async I/O engine");
    } else if (config->enable_iouring) {
        log_message("ENGINE", "io_uring not available, falling back to epoll");
    }
...
int server_stop(Server *server) {
    ...
    for (int i = 0; i < server->thread_count; i++) {
        ...
        if (((pthread_t *)server->thread_pool)[i]) pthread_detach(((pthread_t *)server->thread_pool)[i]);
    }
    pthread_mutex_lock(&connection_mutex);
...
static int read_request_data(Server *server, ConnectionInfo *info, int client_fd, char **out_data, size_t *out_size) {
    char tmp[65536];
    ssize_t n = recv(client_fd, tmp, sizeof(tmp), 0);
    if (n <= 0) return -1;
    server->stats.bytes_received += n;
    info->bytes_received += n;
    info->last_activity = time(NULL);
    if (ensure_capacity(&info->partial_data, &info->partial_capacity, info->partial_size + n + 1) != 0) return -1;
    memcpy_dispatch(info->partial_data + info->partial_size, tmp, n);
    info->partial_size += n;
...
static int stream_ai_response_worker(Server *server, int client_fd, const char *prompt, const char *model_name) {
    ...
    send(client_fd, sse_id, strlen(sse_id), 0);

    char sse_event[] = "event: message\r\n";
    send(client_fd, sse_event, strlen(sse_event), 0);
    ...
    if (result == 0 && ai_buf[0]) {
        ...
        send(client_fd, sse_model, strlen(sse_model), 0);
        ...
        while (pos < total) {
            ...
            if (send(client_fd, sse_data, sse_len, 0) < 0) break;
            pos += to_send;
        ...
        send(client_fd, sse_error, strlen(sse_error), 0);
    }

    char sse_done[] = "data: [DONE]\r\n\r\n";
    send(client_fd, sse_done, strlen(sse_done), 0);
    free(ai_buf);
...
static int http_parser_to_request(HTTPParser *parser, const char *raw, size_t raw_len, HTTPRequest *req) {
    ...
    if (parser->body_start && parser->body_len > 0) {
        ...
        req->body_length = parser->body_len;
    }
...
int server_handle_request(Server *server, int client_fd) {
    ...
    int ret = read_request_data(server, info, client_fd, &request_data, &request_size);
    if (ret == -2) {
        const char *err = "HTTP/1.1 413 Payload Too Large\r\nContent-Length: 0\r\nConnection: close\r\n\r\n";
        send(client_fd, err, strlen(err), 0);
        free_partial_data(info); return -1;
    }
    ...
    if (firewall_is_blacklisted(info->ip_address)) {
        info->flagged_suspicious = 1;
        free_http_request(&request); free_partial_data(info); return -1;
    }
    ...
    if (server->use_streaming && (int)request.method == (int)HTTP_POST && request.path &&
        ...
        if (!prompt && request.body) {
            size_t cl = request.body_length < sizeof(prompt_buf) - 1 ? request.body_length : sizeof(prompt_buf) - 1;
    ...
    if (keep_alive && response.status_code == 200) {
        ...
        if (conn_close_ptr) {
            ptrdiff_t offset = conn_close_ptr - response.data;
            char *new_data = realloc(response.data, response.length + 5);
            if (new_data) {
                response.data = new_data;
                conn_close_ptr = new_data + offset;
                memmove_dispatch(conn_close_ptr + 22, conn_close_ptr + 17, strlen(conn_close_ptr + 17) + 1);
                memcpy_dispatch(conn_close_ptr, "Connection: keep-alive", 22);
                response.length += 5;
            } else keep_alive = 0;
        }
...
int server_send_response(Server *server, int client_fd, const char *response, size_t length) {
    ssize_t bytes_sent = send(client_fd, response, length, MSG_NOSIGNAL);
    if (bytes_sent < 0) return -1;
    server->stats.bytes_sent += bytes_sent;
    ConnectionInfo *info = find_connection_info(client_fd);
    if (info) { info->bytes_sent += bytes_sent; info->last_activity = time(NULL); }
    return 0;
}
